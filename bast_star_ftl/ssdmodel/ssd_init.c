...
void ssd_element_metadata_init(int elem_number, ssd_element_metadata *metadata, ssd_t *currdisk)
{
    ...
    metadata->tot_free_blocks = tot_blocks;
    ...
    for (i = 0; i < (unsigned int)currdisk->params.planes_per_pkg; i ++) {
		...
		metadata->plane_meta[i].active_block = blocks_to_skip;
		metadata->plane_meta[i].active_page = metadata->plane_meta[i].active_block * currdisk->params.pages_per_block;
    ...
    switch(plane_block_mapping) {
        case PLANE_BLOCKS_CONCAT:
			...
			metadata->active_block = 0;
            break;

        case PLANE_BLOCKS_PAIRWISE_STRIPE:
            //metadata->active_block = (2 * usable_blocks_per_plane);
			metadata->active_block = 0;
            break;
	...
	currdisk->data_pages_per_elem = export_size * currdisk->params.pages_per_block;
    ...
    if ((metadata->lba_table = (int *)malloc(usable_blocks * sizeof(int))) == NULL) {
    ...
    // allocate log_block table
    if ((metadata->log_data = (log_table *)malloc(reserved_blocks * sizeof(log_table))) == NULL) {
        fprintf(stderr, "Error: malloc to lba table in ssd_element_metadata_init failed\n");
        fprintf(stderr, "Allocation size = %d\n", reserved_blocks * sizeof(log_table));
        exit(1);
    }

    for ( i = 0 ; i < reserved_blocks ; i++ ){
		metadata->log_data[i].page = (int *)malloc(currdisk->params.pages_per_block * sizeof(int));
    }

	// initialize table
	for ( i = 0 ; i < usable_blocks ; i++) {
		metadata->lba_table[i] = -1;
	}

	for ( i = 0 ; i < reserved_blocks ; i++) {
		int j;
		metadata->log_data[i].bsn = -1;
		metadata->log_data[i].data_block = -1;
		for( j = 0 ; j < currdisk->params.pages_per_block ; j++) {
			metadata->log_data[i].page[j] = -1;
		}
	}
    //////////////////////////////////////////////////////////////////////////////
    ...
    for (i = 0; i < tot_blocks; i ++) {
		...
		// init log_index to -1 
		metadata->block_usage[i].log_index = -1;
    }
