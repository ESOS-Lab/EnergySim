...
int ssd_logical_pageno(int blkno, ssd_t *s)
{
    ...
    lpn = ((abn - (abn % (s->params.element_stride_pages * s->params.nelements)))/
...
{
    double cost;
	int o_block = metadata->lbn_table[lbn];
	...
	//find clean page
	while(metadata->block_usage[o_block].page[c_page] != -1){
		c_page++;
		if(c_page == s->params.pages_per_block) // i think this code don't need
			break;
	}
...
int	_ssd_alloc_log_block(int plane_num, int elem_num, ssd_t *s){
	ssd_element_metadata *metadata = &(s->elements[elem_num].metadata);
    unsigned char *free_blocks = metadata->free_blocks;
    int active_block = -1;
    int prev_pos;
...
listnode **ssd_pick_parunits(ssd_req **reqs, int total, int elem_num, ssd_element_metadata *metadata, ssd_t *s)
{
    ...
    for (i = 0; i < total; i ++) {
        if (reqs[i]->is_read) {
            ...
            ASSERT(prev_block != -1);
            prev_page = (r->blk/s->params.page_size)%s->params.pages_per_block;
    ...
    for (i = 0; i < total; i ++) {
        ...
        if (!reqs[i]->is_read) {
            ...
            plane_num = -1;
            prev_block = metadata->active_block;
            prev_page = (r->blk/s->params.page_size)%s->params.pages_per_block;
            prev_bsn = metadata->block_usage[prev_block].bsn;

			// get the logical page number corresponding to this blkno
			lbn = ssd_logical_pageno(blkno, s);
			offset = (blkno/s->params.page_size)%s->params.pages_per_block;

			// check lbn table
			if(metadata->lba_table[lbn] == -1 ) {
				metadata->lba_table[lbn] == prev_block;
				plane_num = metadata->block_usage[prev_block].plane_num;
			}
			else { //if already mapped, check log block
				tmp_block = metadata->lba_table[lbn];
				if(metadata->block_usage[tmp_block].page[offset] == -1)
					plane_num = metadata->block_usage[tmp_block].plane_num;
				else {
					if (metadata->block_usage[tmp_block].log_index == -1) {
						metadata->block_usage[tmp_block].log_index = _ssd_alloc_log_block(-1, elem_num, s);
						int index = metadata->block_usage[tmp_block].log_index;
						tmp_block = metadata->log_data[index].bsn;
						plane_num = metadata->block_usage[tmp_block].plane_num;
					}
					else {
						if(_last_page_in_log_block(metadata, s)){
							metadata->block_usage[tmp_block].log_index = _ssd_alloc_log_block(-1, elem_num, s);
						}
						int index = metadata->block_usage[tmp_block].log_index;
						tmp_block = metadata->log_data[index].bsn;
    ...
    while (1) {
        ...
        for (i = 0; i < SSD_PARUNITS_PER_ELEM(s); i ++) {
            ...
            if (size > 0) {
                ...
                // get the request
                r = (ssd_req *)n->data;
                lbn = ssd_logical_pageno(r->blk, s);
				offset = (r->blk/s->params.page_size)%s->params.pages_per_block;
                ...
                if (r->is_read) {
                    ...
                    metadata->active_block = metadata->plane_meta[plane_num].active_page;
