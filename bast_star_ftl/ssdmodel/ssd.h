...
#define SSD_MAX_PLANES_PER_ELEM     32
...
typedef struct _log_data{
	int bsn;	//physical block number
	int data_block;
	int *page;	//save page offset
} log_table;
...
typedef struct _plane_metadata {
	...
	unsigned int active_block;       // this points to the next page to write inside an
                                    // active block.
...
typedef struct _ssd_element_metadata {
    int *lba_table;                 // a table mapping the lba to the physical pages
                                    // on the chip.
	log_table *log_data;
	int num_log;					// number of log blocks
	int log_pos;					// start allocating block from this position
...
/*
* statistics per ssd element for power consumption
*/
typedef struct _ssd_power_element_stat {
	int num_reads;
	int num_writes;
	int num_erase;

	double read_power_consumed;
	double write_power_consumed;
	double erase_power_consumed;
	double bus_power_consumed;

	//tiel
	double acc_time;
} ssd_power_element_stat;

/*
 * statistics per ssd for power consumption
 */
typedef struct _ssd_power_ssd_stat {
	double ssd_bus_power_consumed;
	double ssd_bus_time_consumed;
} ssd_power_ssd_stat;

typedef struct _ssd_power_list {
	double time;
	double energy;
	double power;
	double current;
...
// buffer cache policy
#define SSD_BUFFER_CACHE_NO_USE					0
#define SSD_BUFFER_CACHE_USE_DRAM				1
...
typedef struct _ssd_timing_params {
	...
	//@20090828-Micky:add power consumption parameters
	double	flash_input_voltage;
	double	page_read_current;
	double	page_write_current;
	double	page_erase_current;
	double	flash_idle_current;
	double	flash_bus_current;
	double	cpu_normal_mode_power;
	double	cpu_idle_mode_power;
	double	cpu_slow_mode_power;
	double	ssd_bus_current;
	double	channel_switch_delay;		// time to delay switching channel
	double	page_write_max_latency;      // max time to write a page from chip register
	int			nchannel;							// number of channel


	int		buffer_cache_policy; // (e.g. SSD_BUFFER_CACHE_NO_USE, SSD_BUFFER_CACHE_USE_DRAM)
	double	dram_input_voltage;
	double	dram_active_current;
	double	dram_idle_current;

	double	dram_active_latency; // time to read/write a byte from/to register or external interface IO

	int 	dram_cache_size; // bytes
...
typedef struct ssd {
	...
	double overhead; // micky:not used
	...
	int printstats;
	ssd_stat_t stat;
	ssd_power_ssd_stat ssd_power_stat;
	// add tiel
	ssd_power_section power_section;
	ssd_power_section *end_list;
	double acc_time;
	double prev_time;
	double prev_energy;
	double prev_acc_time;
	double section;
	double current_cost;
	double prev_cost;
...
double  _ssd_write_page_osr(ssd_t *s, ssd_element_metadata *metadata, int lpn, ssd_power_element_stat *power_stat);
