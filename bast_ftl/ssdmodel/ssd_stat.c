...
static void ssd_printcleanstats(int *set, int setsize, char *sourcestr)
{
    ...
    for (i = 0; i < setsize; i ++) {
        ...
        ssd_t *s = getssd(set[i]);
            ...
            for (j = 0; j < s->params.nelements; j ++) {
				...
				avg_lifetime = (double)ssd_compute_avg_lifetime(-1, j, s);
            ...
            tot_ssd += tot_elts;
            fprintf(outputfile, "\n");
			t_full += stat->num_fullmerge;
			t_sw += stat->num_switch;
			t_re += stat->num_replacement;
        }
    }
	...
	fprintf(outputfile, "%s   Total Page moved:\t%d\n",
		sourcestr, t_pmove);
	fprintf(outputfile, "%s   Total Number of fullmerge cleans:\t%d\n",
		sourcestr, t_full);
	fprintf(outputfile, "%s   Total Number of switch cleans:\t%d\n",
		sourcestr, t_sw);
	fprintf(outputfile, "%s   Total Number of replacement cleans:\t%d\n",
		sourcestr, t_re);
	...
	for (i = 0; i < setsize; i ++) {
...

		{
			...
			fprintf(outputfile, "%s #%d RAM:     Idle energy:\t%f mJ (%.2f%%) - time taken: %f\n",
				sourcestr, set[i], (s->params.dram_input_voltage * s->params.dram_idle_current * simtime), 100.0, simtime);
			fprintf(outputfile, "\n");
		}
...
//		total_energy += s->params.ssd_bus_current * 5 * simtime;
	...
	for (i = 0; i < setsize; i ++) {
...

		{
			...
			fprintf(outputfile, "%s #%d Flash:     Erase operation energy:\t%f mJ (%.2f%%)\n",
				sourcestr, set[i], element_erase_energy, (element_erase_energy / telement_total_energy * 100.0));
			fprintf(outputfile, "%s #%d Flash:     Bus transfer energy:\t%f mJ (%.2f%%)\n",
