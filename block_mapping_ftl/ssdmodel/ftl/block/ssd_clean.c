...
//            ASSERT(metadata->plane_meta[plane_num].active_page == metadata->active_page);
    ...
    cost += _ssd_write_page_osr(s, metadata, lpn, power_stat, from_blk);
...
(int lb_num, int pp_index, int blk, int plane_num, int elem_num, ssd_element_metadata *metadata, ssd_t *s)
{
    double cost = 0;
    double xfer_cost = 0;
    ssd_power_element_stat *power_stat = &(s->elements[elem_num].power_stat);

	int block = metadata->lba_table[lb_num];

    cost += s->params.page_read_latency * metadata->block_usage[block].num_valid;
    //Micky:add the power consumption of the read
	ssd_power_flash_calculate(SSD_POWER_FLASH_READ, s->params.page_read_latency, power_stat, s);

    cost += ssd_move_page(lb_num, blk, plane_num, elem_num, s);
    ...
    xfer_cost = ssd_crossover_cost(s, metadata, power_stat, blk, SSD_PAGE_TO_BLOCK(metadata->active_block, s));
...
static double _ssd_clean_block_partially(int plane_num, int elem_num, ssd_t *s)
{
    ...
    if (metadata->block_usage[block].num_valid > 0) {
        ...
        int lb_num = metadata->block_usage[block].page[i];

        if (lb_num != -1) {
             cost += ssd_clean_one_page(lb_num, i, block, plane_num, elem_num, metadata, s);
        }
    }
